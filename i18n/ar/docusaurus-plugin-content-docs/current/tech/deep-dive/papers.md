...
<p className="key-answer" data-question="ما هي الأوراق البحثية الرئيسية التي يجب قراءتها في مجال الذكاء الاصطناعي للغو؟">تشمل الأوراق المعلَمية في تاريخ الذكاء الاصطناعي للغو: ورقة MCTS لكولوم (2006)، وAlphaGo (2016)، وAlphaGo Zero (2017)، وAlphaZero المعمَّمة (2017)، وKataGo لديفيد وو (2019) التي قدّمت تحسينات عديدة في الكفاءة؛ لفهم الأساسيات يُنصح بقراءة AlphaGo أولاً، أما للمرجع العملي في التنفيذ فتُعتمد ورقة KataGo بشكل رئيسي.</p>
...
flowchart TB
    subgraph AlphaGo["بنية AlphaGo"]
        subgraph SL["Policy Network - SL"]
            SL1["الإدخال: حالة اللوحة (48 مستوى ميزات)"]
            SL2["البنية: 13 طبقة CNN"]
            SL3["الإخراج: احتمالات 361 موقع"]
            SL4["التدريب: 30 مليون سجل بشري"]
        end

        subgraph RL["Policy Network - RL"]
            RL1["تهيئة من SL Policy"]
            RL2["تعلم معزز باللعب الذاتي"]
        end

        subgraph VN["Value Network"]
            VN1["الإدخال: حالة اللوحة"]
            VN2["الإخراج: قيمة معدل فوز واحدة"]
            VN3["التدريب: أوضاع من اللعب الذاتي"]
        end

        subgraph MCTS["MCTS"]
            M1["استخدام Policy Network لتوجيه البحث"]
            M2["استخدام Value Network + Rollout للتقييم"]
        end
    end
```
...
```mermaid
flowchart TB
    Input["الإدخال (17 مستوى)"]
    Input --> ResNet["البرج المتبقي<br/>(19 أو 39 طبقة)"]
...
```mermaid
flowchart TB
    Init["تهيئة شبكة عشوائية"]
    Init --> SelfPlay["لعب ذاتي لتوليد سجلات"]
    SelfPlay --> Train["تدريب الشبكة العصبية<br/>- Policy: تقليل الانتروبيا المتقاطعة<br/>- Value: تقليل MSE"]
    Train --> Eval["تقييم الشبكة الجديدة<br/>إذا كانت أقوى تحل محلها"]
    Eval -.->|تكرار| SelfPlay
```
