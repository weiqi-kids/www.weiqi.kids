...
```mermaid
flowchart TB
    subgraph Environment["Environment (Entorno)"]
        State["State (Estado)<br/>s_t"]
        Reward["Reward (Recompensa)<br/>r_t"]
    end

    subgraph Agent["Agent (Agente)"]
        Policy["Policy (Politica)<br/>π(s)"]
        Action["Action (Accion)<br/>a_t"]
    end

    State --> Policy
    Reward --> Policy
    Policy --> Action
    Action -->|"Enviar al entorno"| Environment
```
