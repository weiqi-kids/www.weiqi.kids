...
```
Nodo Max A (mejor actual α = +5)
    │
    ├── Rama 1: Ya evaluada, valor = +5
    │
    └── Rama 2: Nodo Min B
            │
            ├── Sub-rama B1: valor = +3
            │
            └── Sub-rama B2: ??? (¡se puede saltar!)
```
...
Donde:
- **X̄(s,a)**: Valor promedio (tasa de victorias) de tomar la accion a en el estado s
