...
```mermaid
flowchart LR
    subgraph Ciclo["Ciclo MCTS (Repetir N veces)"]
        A["Selection<br/>(Seleccion)"] --> B["Expansion<br/>(Expansion)"]
        B --> C["Simulation<br/>(Simulacion)"]
        C --> D["Backprop<br/>(Retropropagacion)"]
        D --> A
    end
```
...
```mermaid
flowchart TB
    subgraph AlphaGo["Arquitectura de Busqueda AlphaGo"]
        subgraph MCTS["MCTS"]
            Selection["Selection<br/>Formula PUCT"]
            Expansion["Expansion<br/>Policy Net"]
            Evaluation["Evaluation<br/>Value Net + Rollout"]
            Backup["Backup<br/>Actualizar"]
        end

        subgraph NN["Neural Networks"]
            PolicyNet["Policy Network<br/>π(a|s)<br/>Salida: prob. de acciones"]
            ValueNet["Value Network<br/>V(s)<br/>Salida: tasa de victoria"]
        end
