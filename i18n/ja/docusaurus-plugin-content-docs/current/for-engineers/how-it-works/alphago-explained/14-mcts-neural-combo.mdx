...
```mermaid
flowchart LR
    subgraph MCTS["MCTS ループ（N 回繰り返す）"]
        A["Selection<br/>選択"] --> B["Expansion<br/>展開"]
        B --> C["Simulation<br/>シミュレーション"]
        C --> D["Backprop<br/>逆伝播"]
        D --> A
    end
```
...
```mermaid
flowchart TB
    subgraph AlphaGo["AlphaGo 探索アーキテクチャ"]
        subgraph MCTS["MCTS"]
            direction LR
            S["Selection"]
            E["Expansion"]
            ...
            S --> P1["PUCT<br/>公式"]
            E --> P2["Policy<br/>Net"]
            EV --> P3["Value Net<br/>+ Rollout"]
            B --> P4["統計<br/>更新"]
        end

        subgraph NN["Neural Networks"]
            direction LR
            PN["Policy Network<br/>π(a|s)<br/>出力：行動確率"]
            VN["Value Network<br/>V(s)<br/>出力：勝率推定"]
        end
...
flowchart TB
    A["AlphaGo Fan (2015)"]
    B["AlphaGo Lee (2016)"]
    C["AlphaGo Master (2017)"]
    D["AlphaGo Zero (2017)"]
    E["AlphaZero (2018)"]

    A -->|"+ より大きなネットワーク<br/>より多くの訓練"| B
    B -->|"+ より多くの自己対局"| C
    C -->|"+ 人間のデータを削除<br/>ネットワークを統一<br/>Rollout を削除"| D
    D -->|"+ 他のゲームへの一般化"| E
```
