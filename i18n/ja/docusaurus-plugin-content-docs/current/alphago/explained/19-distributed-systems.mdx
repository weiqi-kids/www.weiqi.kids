...
```mermaid
flowchart TB
    subgraph Loop["AlphaGo Zero 学習ループ"]
        SelfPlay["Self-play Workers<br/>(TPU × N)"]
        Replay["Replay Buffer<br/>(RAM/SSD)<br/>直近 50 万局"]
        Training["Training Workers<br/>(TPU × M)"]
        Checkpoint["Network Checkpoint"]

        Network["最新ネットワーク"] -.-> SelfPlay
        SelfPlay --> Replay
        Replay --> Training
        Training --> Checkpoint
        Checkpoint -->|"Self-play 用ネットワークを更新"| Network
    end
```
...
flowchart LR
    subgraph MCTS["Parallel MCTS"]
        T1["Thread 1 → リーフノード L1"]
        T2["Thread 2 → リーフノード L2"]
        T3["Thread 3 → リーフノード L3"]
        T4["Thread 4 → リーフノード L4"]
    end

    Batch["Batch"]
    TPU["TPU"]
    Result["(P1,V1), (P2,V2), ...<br/>を同時に取得"]
...
```mermaid
flowchart TB
    subgraph Arch["分散推論アーキテクチャ"]
        Master["主制御ノード<br/>相手の着手を受信、AlphaGo の着手を送信"]

        MCTSCtrl["MCTS コントローラ<br/>探索ツリー管理、タスク配分、結果収集"]

        subgraph TPUCluster["TPU クラスタ（48 個の TPU）"]
            TPU1["TPU 1"]
            TPU2["TPU 2"]
            TPU3["TPU 3"]
            TPU4["TPU 4"]
        ...
        BatchNote["ニューラルネットワーク推論リクエストをバッチ処理"]

        Master --> MCTSCtrl
        MCTSCtrl --> TPUCluster
        TPUCluster --> BatchNote
    end
```
