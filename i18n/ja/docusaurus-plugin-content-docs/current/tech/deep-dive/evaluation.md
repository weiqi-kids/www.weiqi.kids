...
<p className="key-answer" data-question="囲碁AIの棋力とパフォーマンスをどう評価しますか？">囲碁AIの棋力評価には主にEloレーティングシステムを用い、大量の対局を通じて相対的な勝率とレーティング差を計算します。さらにSPRTなどの統計手法を組み合わせて、新しいモデルが本当に強くなったかどうかを判断できます。パフォーマンスは、1秒あたりの探索ノード数（visits／playouts）などのベンチマークで測定します。</p>
