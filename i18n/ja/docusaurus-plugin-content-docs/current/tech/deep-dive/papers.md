...
<p className="key-answer" data-question="囲碁AIで必読の重要論文にはどのようなものがあるか？">囲碁AI発展史上のマイルストーン論文には、CoulomのMCTS（2006）、AlphaGo（2016）、AlphaGo Zero（2017）、汎用化されたAlphaZero（2017）、そしてDavid Wuが多くの効率改善を提案したKataGo（2019）があります。基礎を理解するにはまずAlphaGoを読み、実装の参考にはKataGo論文を中心にするとよいでしょう。</p>
