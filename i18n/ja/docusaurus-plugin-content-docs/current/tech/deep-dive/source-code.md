...
<p className="key-answer" data-question="KataGoのソースコード構造はどのようになっているのか？">KataGoのソースコード（github.com/lightvector/KataGo）は主に2つの部分に分かれています。cpp/ はC++のコアエンジンで、メインプログラム、囲碁ルール（game）、MCTS探索（search）、ニューラルネットワーク推論（neuralnet）を含みます。python/ はPythonの訓練コードで、訓練メインプログラム、ネットワークアーキテクチャ定義（model.py）、データ処理を含みます。</p>
