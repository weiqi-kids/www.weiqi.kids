...
<p className="key-answer" data-question="KataGoの分散訓練アーキテクチャはどう動作しますか？">KataGoの分散訓練では、世界中のボランティアのSelf-play Workerが自己対局を実行して棋譜データをアップロードし、中央の訓練サーバーがデータを収集して定期的により強いモデルを訓練し、それをWorkerに公開して対局を続けることで、継続的に自己改善するループを形成します。</p>
...
```mermaid
flowchart TB
    subgraph System["KataGo 分散訓練システム"]
        ...
        Server["訓練サーバー<br/>(Data Collection)"]
        Server --> Training["訓練フロー<br/>(Model Training)"]
        Training --> Release["新モデル公開<br/>(Model Release)"]
    end
```
