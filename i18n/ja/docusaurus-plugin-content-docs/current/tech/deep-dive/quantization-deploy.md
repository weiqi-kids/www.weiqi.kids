...
<p className="key-answer" data-question="KataGoモデルをどのように量子化してデプロイするのか？">量子化は数値精度を下げることでモデルサイズを削減し、推論を高速化します。FP16はメモリを半分に抑えつつほぼ精度損失がなく、INT8はさらに4分の1まで圧縮できます。量子化方式は、簡単で高速な訓練後量子化（PTQ）と、精度は高いが再訓練が必要な量子化感知訓練（QAT）に分かれ、TensorRTなどのツールと組み合わせて各プラットフォームへデプロイします。</p>
