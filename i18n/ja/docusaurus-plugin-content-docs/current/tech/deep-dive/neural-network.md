...
<p className="key-answer" data-question="KataGoのニューラルネットワークアーキテクチャとは何か？">KataGoは単一ニューラルネットワーク・マルチヘッド出力の設計を採用しています。残差タワー（20〜60個の残差ブロックにグローバルプーリング層を加えたもの）を主幹として入力特徴プレーンから特徴を抽出し、そこからPolicy（着手確率）、Value（勝率）、Score（目数差）、Ownership（地の帰属）の4つの出力ヘッドに分岐します。</p>
