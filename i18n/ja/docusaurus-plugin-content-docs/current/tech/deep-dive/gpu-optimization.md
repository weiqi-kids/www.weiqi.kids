...
<p className="key-answer" data-question="KataGoのGPUパフォーマンスをどう最適化しますか？">KataGoはCUDA、OpenCL、Metal、Eigenの4種類の計算バックエンドを提供しています。NVIDIA GPUではCUDAが最高のパフォーマンスを発揮し、メーカーを問わず使えるのはOpenCL、Apple SiliconではMetal、CPUのみの場合はEigenを使います。最適化の鍵は、適切なバックエンドを選び、バッチサイズ（nnMaxBatchSize）とスレッド数を調整してGPUを十分に活用することです。</p>
