...
```mermaid
flowchart TB
    subgraph Phase1["阶段 1：监督学习"]
        A1["人类棋谱<br/>(3000万局)"] --> B1["GPU 集群<br/>(50 GPUs)"] --> C1["Policy Net<br/>(SL版)"]
    end

    subgraph Phase2["阶段 2：强化学习"]
        A2["自我对弈<br/>(数百万局)"] --> B2["GPU 集群<br/>(50 GPUs)"] --> C2["Policy Net<br/>(RL版)"]
    end

    subgraph Phase3["阶段 3：价值网络训练"]
        A3["RL对弈数据<br/>(3000万局面)"] --> B3["GPU 集群<br/>(50 GPUs)"] --> C3["Value Net"]
    end
...
```mermaid
flowchart TB
    subgraph Loop["AlphaGo Zero 训练循环"]
        SP["Self-play Workers<br/>(TPU × N)"] --> RB["Replay Buffer<br/>(RAM/SSD)<br/>最近 50 万局"]
        RB --> TW["Training Workers<br/>(TPU × M)"]
        TW --> NC["Network Checkpoint"]
        NC -->|"更新网络"| SP
    end
```
...
flowchart TB
    Root["Root"]
    Root --> Sim1["Sim 1<br/>(独立)"]
    Root --> Sim2["Sim 2<br/>(独立)"]
    Root --> Sim3["Sim 3<br/>(独立)"]
    Root --> Sim4["Sim 4<br/>(独立)"]
...
```mermaid
flowchart TB
    subgraph Arch["分布式推理架构"]
        Master["主控节点<br/>接收对手落子，发送 AlphaGo 落子"]
        Master --> Controller["MCTS 控制器<br/>管理搜索树、分配任务、收集结果"]
        Controller --> TPUCluster

        subgraph TPUCluster["TPU 集群（48 个 TPU）"]
            TPU1["TPU 1"]
            TPU2["TPU 2"]
            TPU3["TPU 3"]
            TPU4["TPU 4"]
