...
<p className="key-answer" data-question="KataGo 的训练机制如何运作？">KataGo 采用自我对弈强化学习：以初始模型大量自我对弈生成棋谱，收集数据后用策略与价值损失函数训练更新出更强的模型，再重复此循环持续进步；训练所需 GPU 内存与时间随模型规模（如 b6c96 到 b40c256）递增。</p>
