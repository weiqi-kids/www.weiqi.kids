...
<p className="key-answer" data-question="KataGo 的分布式训练架构如何运作？">KataGo 分布式训练由全球志愿者的 Self-play Worker 执行自我对弈并上传棋谱数据，中央训练服务器收集数据后定期训练出更强的模型，再发布给 Worker 继续对弈，形成持续自我改进的循环。</p>
...
```mermaid
flowchart TB
    subgraph KataGo["KataGo 分布式训练系统"]
        ...
        Server["训练服务器<br/>(Data Collection)"]
        Training["训练流程<br/>(Model Training)"]
        Release["新模型发布<br/>(Model Release)"]
