...
<p className="key-answer" data-question="KataGo 的神经网络架构是什么？">KataGo 采用单一神经网络、多头输出的设计：以残差塔（20–60 个残差块并加入全局池化层）为主干，从输入特征平面提取特征，再分出 Policy（落子概率）、Value（胜率）、Score（目数差）与 Ownership（领地归属）四个输出头。</p>
