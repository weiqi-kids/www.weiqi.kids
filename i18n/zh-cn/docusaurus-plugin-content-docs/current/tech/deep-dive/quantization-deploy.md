...
<p className="key-answer" data-question="如何量化并部署 KataGo 模型？">量化通过降低数值精度减少模型大小并加速推理：FP16 可省一半内存且几乎无精度损失，INT8 可再压缩至四分之一；量化方式分为训练后量化（PTQ，简单快速）与量化感知训练（QAT，精度更高但需重新训练），再搭配 TensorRT 等工具部署到各平台。</p>
