...
<p className="key-answer" data-question="KataGo 的 MCTS 是如何实现的？">KataGo 的 MCTS 由选择（Selection）、扩展（Expansion）、评估（Evaluation）、回传（Backpropagation）四步骤循环组成，选择阶段以 PUCT 公式平衡探索与利用，并通过虚拟损失、批量神经网络评估与多线程并行化大幅提升搜索效率。</p>
...
```mermaid
flowchart TB
    subgraph MCTS["MCTS 搜索循环"]
        S["1. Selection<br/>选择：沿树向下，用 PUCT 选节点"]
        E["2. Expansion<br/>展开：到达叶节点，创建子节点"]
        V["3. Evaluation<br/>评估：用神经网络评估叶节点"]
        B["4. Backprop<br/>回传：更新路径上所有节点统计"]
        R["重复数千次，选择访问次数最多的动作"]
