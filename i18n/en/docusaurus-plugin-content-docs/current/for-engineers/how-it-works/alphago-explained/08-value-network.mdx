...
```
Input Layer → Conv Layers ×12 → Fully Connected → Output
    ↓              ↓                  ↓            ↓
 19×19×48      19×19×192            256-dim    Single Value
```
...
Input: 19×19×49
    ...
    Flatten (361-dim)
        ↓
    Fully Connected (256-dim)
        ↓
    ReLU
        ↓
    Fully Connected (1-dim)
        ↓
    Tanh
        ↓
Output: [-1, +1]
...
v3 = V(leaf) = 0.6
      ↑
A2's Q value updates
      ↑
A's Q value updates
      ↑
Root node statistics update
```
...
```
  ...
  Policy Head    Value Head
       ↓              ↓
  361 probabilities  Single value
```
