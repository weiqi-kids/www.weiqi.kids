...
```mermaid
flowchart LR
    S["Selection<br/>Seleksi"]
    E["Expansion<br/>Ekspansi"]
    Sim["Simulation<br/>Simulasi"]
    B["Backprop<br/>Propagasi"]
...
flowchart TB
    subgraph MCTS["MCTS"]
        Selection["Selection<br/>PUCT Rumus"]
    ...
    subgraph NN["Neural Networks"]
        Policy["Policy Network<br/>π(a|s)<br/>Output: prob. aksi"]
        Value["Value Network<br/>V(s)<br/>Output: win rate estimasi"]
    end
...
flowchart TB
    ...
    Fan -->|"+ Network lebih besar, lebih banyak pelatihan"| Lee
    Lee -->|"+ Lebih banyak self-play"| Master
    Master -->|"+ Hapus data manusia, unifikasi network, hapus Rollout"| Zero
    Zero -->|"+ Generalisasi ke game lain"| AZ
```
...
- **Artikel Berikutnya**: [Penjelasan Detail Rumus PUCT](../puct-formula) — Prinsip matematika seleksi MCTS
- **Artikel Sebelumnya**: [Self-Play](../self-play) — Mekanisme dan efek self-play
- **Terkait**: [Penjelasan Detail Policy Network](../policy-network) — Arsitektur strategy network
