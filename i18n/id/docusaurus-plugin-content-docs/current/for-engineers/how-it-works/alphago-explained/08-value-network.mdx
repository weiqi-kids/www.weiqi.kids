...
```
Layer Input → Layer Konvolusi ×12 → Layer Fully Connected → Output
   ↓         ↓           ↓         ↓
19×19×48   19×19×192    256 dimensi     Nilai tunggal
```
...
Input: 19×19×49
    ...
    Flatten (361 dimensi)
        ↓
    Fully Connected (256 dimensi)
        ↓
    ReLU
        ↓
    Fully Connected (1 dimensi)
        ↓
    Tanh
        ↓
Output: [-1, +1]
...
```
Pohon pencarian MCTS:

        Root Node (posisi saat ini)
           /    \
         A        B
        /  \    /  \
       A1  A2  B1  B2 ← Leaf node
        ↓   ↓   ↓   ↓
       Eval  Eval  Eval  Eval
```
...
```
       Layer ekstraksi fitur bersama
              |
       ┌──────┴──────┐
       ↓              ↓
  Policy Head    Value Head
       ↓              ↓
  361 probabilitas   Nilai tunggal
```
