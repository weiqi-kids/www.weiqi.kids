...
flowchart TB
    ...
    Input --> Conv --> BN --> ReLU --> Flat --> FC --> Soft --> Out
```
...
flowchart TB
    ...
    Input --> Conv --> BN --> R1 --> Flat --> FC1 --> R2 --> FC2 --> Tanh --> Out
```
...
- **Artikel Sebelumnya**: [Ikhtisar AlphaGo Zero](../alphago-zero) — Mengapa tidak memerlukan catatan permainan manusia
- **Artikel Berikutnya**: [Proses Pelatihan dari Nol](../training-from-scratch) — Evolusi detail Hari 0-3
- **Teknis Mendalam**: [CNN dan Go](../cnn-and-go) — Mengapa CNN cocok untuk papan
