...
```mermaid
flowchart TB
    subgraph S1["Tahap 1: Supervised Learning"]
        K1["Kifu Manusia<br/>(30 juta)"]
        G1["GPU Cluster<br/>(50 GPUs)"]
        P1["Policy Net<br/>(versi SL)"]
        K1 --> G1 --> P1
    end

    subgraph S2["Tahap 2: Reinforcement Learning"]
        SP["Self-play<br/>(jutaan)"]
        G2["GPU Cluster<br/>(50 GPUs)"]
        P2["Policy Net<br/>(versi RL)"]
        SP --> G2 --> P2
    end

    subgraph S3["Tahap 3: Pelatihan Value Network"]
        D["Data RL<br/>(30 juta posisi)"]
        G3["GPU Cluster<br/>(50 GPUs)"]
        V["Value Net"]
        D --> G3 --> V
    end
...
flowchart TB
    ...
    RB["Replay Buffer<br/>(500.000 pertandingan terakhir)<br/>(RAM/SSD)"]
    TW["Training Workers<br/>(TPU × M)"]
    CK["Network Checkpoint"]

    SP -->|Data pertandingan| RB
    RB -->|Sampel pelatihan| TW
    TW -->|Perbarui| CK
    CK -->|Network terbaru| SP
```
...
```mermaid
flowchart LR
    subgraph Threads["Parallel MCTS"]
        T1["Thread 1 → Leaf L1"]
        T2["Thread 2 → Leaf L2"]
        T3["Thread 3 → Leaf L3"]
        T4["Thread 4 → Leaf L4"]
...
```mermaid
flowchart TB
    Node["Node Utama<br/>Terima langkah lawan, kirim langkah AlphaGo"]
    MCTS["MCTS Controller<br/>Kelola search tree, distribusi tugas, kumpulkan hasil"]

    subgraph Cluster["Cluster TPU (48 TPU)"]
        TPU1["TPU 1"]
        TPU2["TPU 2"]
        TPU3["TPU 3"]
        TPU4["TPU 4"]
...
- **Artikel sebelumnya**: [Proses Pelatihan dari Nol](../training-from-scratch) — Analisis detail kurva pelatihan
- **Artikel selanjutnya**: [Warisan AlphaGo](../legacy-and-impact) — Dampak mendalam AlphaGo pada bidang AI
- **Artikel terkait**: [Kombinasi MCTS dan Neural Network](../mcts-neural-combo) — Pengetahuan dasar MCTS
