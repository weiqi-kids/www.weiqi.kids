---
sidebar_position: 18
title: Rede de Cabeça Dupla e Rede Residual
description: Análise aprofundada da arquitetura de rede neural do AlphaGo Zero - Backbone compartilhado, Policy Head, Value Head e ResNet de 40 camadas
keywords: [rede de cabeça dupla, rede residual, ResNet, Policy Head, Value Head, aprendizado profundo, arquitetura de rede neural]
---
...
flowchart TB
    ...
    Backbone --> PH["Policy Head<br/>(Cabeça de Política)"]
    Backbone --> VH["Value Head<br/>(Cabeça de Valor)"]

    PH --> POut["Distribuição de probabilidade<br/>19×19 + 1 Pass"]
    VH --> VOut["Taxa de vitória<br/>[-1, 1]"]
```
...
flowchart TB
    X["Entrada x"]
    X --> Conv["Camada conv<br/>BN + ReLU<br/>Camada conv<br/>BN"]
    Conv --> FX["F(x)"]
...
- **Artigo anterior**: [Visão Geral do AlphaGo Zero](../alphago-zero) — Por que não precisa de registros de partidas humanas
- **Próximo artigo**: [O Processo de Treinamento do Zero](../training-from-scratch) — Evolução detalhada dos Dias 0-3
- **Aprofundamento técnico**: [CNN e Go Combinados](../cnn-and-go) — Por que CNN é adequada para tabuleiros
