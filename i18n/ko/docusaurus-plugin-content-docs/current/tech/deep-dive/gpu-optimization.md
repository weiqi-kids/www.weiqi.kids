...
<p className="key-answer" data-question="KataGo의 GPU 성능은 어떻게 최적화하나요?">KataGo는 CUDA, OpenCL, Metal, Eigen 네 가지 계산 백엔드를 제공합니다. NVIDIA GPU는 CUDA가 가장 빠르고, 여러 제조사를 아우르려면 OpenCL, Apple Silicon은 Metal, 순수 CPU는 Eigen을 사용합니다. 최적화의 핵심은 알맞은 백엔드를 고르고 배치 크기(nnMaxBatchSize)와 스레드 수를 조정해 GPU를 충분히 활용하는 것입니다.</p>
