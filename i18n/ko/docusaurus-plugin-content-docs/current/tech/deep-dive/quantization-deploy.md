...
<p className="key-answer" data-question="KataGo 모델을 어떻게 양자화하고 배포하나요?">양자화는 수치 정밀도를 낮춰 모델 크기를 줄이고 추론을 가속합니다. FP16은 메모리를 절반으로 줄이면서 정밀도 손실이 거의 없고, INT8은 다시 4분의 1까지 압축할 수 있습니다. 양자화 방식은 학습 후 양자화(PTQ, 간단하고 빠름)와 양자화 인식 학습(QAT, 정밀도가 더 높지만 재학습 필요)으로 나뉘며, TensorRT 같은 도구와 함께 각 플랫폼에 배포합니다.</p>
