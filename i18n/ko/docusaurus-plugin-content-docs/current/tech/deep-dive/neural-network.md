...
<p className="key-answer" data-question="KataGo의 신경망 아키텍처는 무엇인가요?">KataGo는 단일 신경망에 다중 헤드 출력을 결합한 설계를 채택합니다. 잔차 타워(20~60개의 잔차 블록에 전역 풀링층을 더한 구조)를 주축으로 입력 특성 평면에서 특징을 추출한 뒤, Policy(착수 확률), Value(승률), Score(집 차이), Ownership(영역 귀속) 네 개의 출력 헤드로 나뉩니다.</p>
