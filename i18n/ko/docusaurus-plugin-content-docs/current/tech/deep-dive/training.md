...
<p className="key-answer" data-question="KataGo의 학습 메커니즘은 어떻게 작동하나요?">KataGo는 자가 대국 강화 학습을 채택합니다. 초기 모델로 대량의 자가 대국을 진행해 기보를 생성하고, 수집한 데이터로 정책 손실과 가치 손실 함수를 사용해 더 강한 모델로 업데이트한 뒤 이 순환을 반복하며 계속 성장합니다. 학습에 필요한 GPU 메모리와 시간은 모델 규모(예: b6c96에서 b40c256까지)가 커질수록 증가합니다.</p>
