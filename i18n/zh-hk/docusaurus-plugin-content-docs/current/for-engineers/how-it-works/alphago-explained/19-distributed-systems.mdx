...
```mermaid
flowchart TB
    subgraph Phase1["階段 1：監督學習"]
        P1Data["人類棋譜<br/>(3000萬局)"]
        P1GPU["GPU 集群<br/>(50 GPUs)"]
        P1Net["Policy Net<br/>(SL版)"]
        P1Data --> P1GPU --> P1Net
    end

    subgraph Phase2["階段 2：強化學習"]
        P2Data["自我對弈<br/>(數百萬局)"]
        P2GPU["GPU 集群<br/>(50 GPUs)"]
        P2Net["Policy Net<br/>(RL版)"]
        P2Data --> P2GPU --> P2Net
    end

    subgraph Phase3["階段 3：價值網絡訓練"]
        P3Data["RL對弈資料<br/>(3000萬局面)"]
        P3GPU["GPU 集群<br/>(50 GPUs)"]
        P3Net["Value Net"]
        P3Data --> P3GPU --> P3Net
    end
...
flowchart TB
    subgraph Loop["AlphaGo Zero 訓練循環"]
        ...
        Buffer --> Training
        Training --> Checkpoint
        Checkpoint --> |"更新 Self-play 用嘅網絡"| SelfPlay
    end

    LatestNet["最新網絡"] --> SelfPlay
```
...
flowchart TB
    subgraph Inference["分散式推理架構"]
        ...
        Master --> MCTS["MCTS 控制器<br/>管理搜索樹、分配任務、收集結果"]
...
- **上一篇**：[從零訓練嘅過程](../training-from-scratch) — 訓練曲線嘅詳細分析
- **下一篇**：[AlphaGo 嘅遺產](../legacy-and-impact) — AlphaGo 對 AI 領域嘅深遠影響
- **相關文章**：[MCTS 與神經網絡嘅結合](../mcts-neural-combo) — MCTS 嘅基礎知識
