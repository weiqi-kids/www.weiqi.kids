...
sidebar_position: 8
title: 模型量化與部署
description: KataGo 模型嘅量化技術、匯出格式同各平台部署方案
---

# 模型量化與部署

本文介紹點樣將 KataGo 模型量化以減少資源需求，以及喺各種平台上嘅部署方案。

<p className="key-answer" data-question="點樣量化並部署 KataGo 模型？">量化透過降低數值精度減少模型大小同加速推理：FP16 可省一半記憶體而且幾乎無精度損失，INT8 可再壓縮至四分之一；量化方式分訓練後量化（PTQ，簡單快速）同量化感知訓練（QAT，精度更高但需重新訓練），再配搭 TensorRT 等工具部署到各平台。</p>
