...
<p className="key-answer" data-question="KataGo 嘅 MCTS 係點樣實作嘅？">KataGo 嘅 MCTS 由選擇（Selection）、展開（Expansion）、評估（Evaluation）、回傳（Backpropagation）四步驟循環組成，選擇階段以 PUCT 公式平衡探索同利用，並透過虛擬損失、批次神經網絡評估同多執行緒並行化大幅提升搜索效率。</p>
