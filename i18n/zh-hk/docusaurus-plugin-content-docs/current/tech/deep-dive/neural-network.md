...
sidebar_position: 4
title: 神經網絡架構詳解
description: 深入解析 KataGo 嘅神經網絡設計、輸入特徵同多頭輸出架構
---

# 神經網絡架構詳解

本文深入解析 KataGo 神經網絡嘅完整架構，由輸入特徵編碼到多頭輸出設計。

<p className="key-answer" data-question="KataGo 嘅神經網絡架構係咩？">KataGo 採用單一神經網絡、多頭輸出嘅設計：以殘差塔（20–60 個殘差塊並加入全局池化層）做主幹，由輸入特徵平面提取特徵，再分出 Policy（落子機率）、Value（勝率）、Score（目數差）同 Ownership（領地歸屬）四個輸出頭。</p>

---

## 架構總覽

KataGo 用**單一神經網絡、多頭輸出**嘅設計：

```mermaid
flowchart TB
    Input["輸入特徵（19×19×22）"]
    Input --> Conv
    Conv["初始卷積層<br/>256 filters"]
    Conv --> ResNet
    ResNet["殘差塔<br/>20-60 個殘差塊<br/>+ 全局池化層"]
    ResNet --> PolicyHead["Policy Head"]
    ResNet --> ValueHead["Value Head"]
    ResNet --> ScoreHead["Score Head"]
    ResNet --> OwnershipHead["Ownership Head"]
    PolicyHead --> PolicyOut["362 機率<br/>(含 pass)"]
    ValueHead --> ValueOut["勝率<br/>(-1~+1)"]
    ScoreHead --> ScoreOut["目數差<br/>(目)"]
    OwnershipHead --> OwnershipOut["361 歸屬<br/>(-1~+1)"]
```
...
KataGo 用 **22 個特徵平面**（19×19×22），每個平面係一個 19×19 嘅矩陣：
...
| 1 | 對方棋子 | 1 = 有對方棋子，0 = 冇 |
| 2 | 空點 | 1 = 空點，0 = 有棋子 |
| 3-10 | 歷史狀態 | 過去 8 步嘅棋盤變化 |
| 11 | 劫禁點 | 1 = 呢度係劫禁，0 = 可以落 |
| 12-17 | 氣數編碼 | 1氣、2氣、3氣...嘅棋串 |
| 18-21 | 規則編碼 | 中國/日本規則、komi 等 |

### 歷史狀態堆疊

為咗令神經網絡理解局面嘅**動態變化**，KataGo 會堆疊過去 8 步嘅棋盤狀態：
...
KataGo 用 **Pre-activation ResNet** 結構：
...
KataGo 嘅關鍵創新之一：喺殘差塊入面加入**全局池化**，令網絡可以睇到全局資訊：
...
傳統卷積淨係睇局部（3×3 感受野），就算堆疊好多層，對全局資訊嘅感知仍然有限。全局池化令網絡可以直接「睇到」：
- 成盤棋嘅子數差異
- 全局嘅勢力分佈
- 整體嘅形勢判斷
...
- [MCTS 實作細節](../mcts-implementation) — 搜索同神經網絡嘅結合
- [KataGo 訓練機制解析](../training) — 網絡點樣訓練
- [關鍵論文導讀](../papers) — 原始論文嘅數學推導
