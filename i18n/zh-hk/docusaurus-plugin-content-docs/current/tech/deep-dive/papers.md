...
<p className="key-answer" data-question="圍棋 AI 有邊啲必讀嘅關鍵論文？">圍棋 AI 發展史上嘅里程碑論文包括：Coulom 嘅 MCTS（2006）、AlphaGo（2016）、AlphaGo Zero（2017）、通用化嘅 AlphaZero（2017），以及 David Wu 提出多項效率改進嘅 KataGo（2019）；想了解基礎可以先讀 AlphaGo，實作參考則以 KataGo 論文為主。</p>
...
flowchart TB
    Input["輸入（17 平面）"]
    Input --> ResNet
    subgraph ResNet["殘差塔 (19 or 39 層)"]
        R[" "]
    end
...
flowchart TB
    ...
    SelfPlay["自我對弈產生棋譜"]
    SelfPlay --> Train
    Train["訓練神經網絡<br/>- Policy: 最小化交叉熵<br/>- Value: 最小化 MSE"]
    Train --> Eval
    Eval["評估新網絡<br/>若較強則替換"]
    Eval --> SelfPlay
```
