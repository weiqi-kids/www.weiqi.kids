...
sidebar_position: 7
title: 分散式訓練架構
description: KataGo 分散式訓練系統嘅架構、Self-play Worker 同模型發佈流程
---

# 分散式訓練架構

本文介紹 KataGo 嘅分散式訓練系統架構，講解點樣透過全球社群嘅算力持續改進模型。

<p className="key-answer" data-question="KataGo 嘅分散式訓練架構點樣運作？">KataGo 分散式訓練由全球志願者嘅 Self-play Worker 執行自我對弈並上傳棋譜資料，中央訓練伺服器收集資料之後定期訓練出更強模型，再發布畀 Worker 繼續對弈，形成持續自我改進嘅循環。</p>
...
flowchart TB
    subgraph System["KataGo 分散式訓練系統"]
        ...
        Train["訓練流程<br/>(Model Training)"]
        Release["新模型發佈<br/>(Model Release)"]
