...
sidebar_position: 16
title: PUCT 公式詳解
description: 深入理解 AlphaGo 嘅核心揀選機制：PUCT 公式嘅數學原理與調參經驗
---
...
| 符號 | 意義 | 來源 |
|------|------|------|
| $Q(s,a)$ | 動作 $a$ 嘅平均價值 | MCTS 統計 |
| $P(s,a)$ | 動作 $a$ 嘅先驗機率 | Policy Network |
| $N(s)$ | 父節點嘅訪問次數 | MCTS 統計 |
| $N(s,a)$ | 子節點嘅訪問次數 | MCTS 統計 |
...
$Q(s,a)$ 係由節點 $(s,a)$ 出發嘅所有模擬嘅平均結果：

$$Q(s,a) = \frac{W(s,a)}{N(s,a)} = \frac{\sum_i z_i}{N(s,a)}$$

其中 $z_i \in \{-1, +1\}$ 係第 $i$ 次模擬嘅結果。
...
**解讀**：
- $Q = 0.6$：呢步棋嘅勝率約 80%（因為 $Q = 2 \times \text{勝率} - 1$）
- $Q = 0$：勝負各半
- $Q = -0.3$：呢步棋嘅勝率約 35%

### P(s,a)：先驗機率

$P(s,a)$ 嚟自 Policy Network 嘅輸出：
...
呢個分數嘅行為：
- 當 $N(s,a) = 0$ 嗰陣，分數 = $\sqrt{N(s)}$（最大探索動力）
- 隨住 $N(s,a)$ 增加，分數下降
- 當 $N(s,a) \gg \sqrt{N(s)}$ 嗰陣，分數趨近於 0
...
AlphaGo 使用嘅值：$c_{\text{puct}} = 1.5$（根據論文）。
...
1. **利用先驗知識**：Policy Network 提供嘅 $P(s,a)$ 令搜索一開始就聚焦喺合理嘅著法上面

2. **更快嘅收斂**：線性衰減（$1/(1+n)$）比對數衰減（$1/\sqrt{\ln N / n}$）更快令搜索聚焦

3. **可調控嘅探索**：$P(s,a)$ 同 $c_{\text{puct}}$ 提供咗更多調控探索嘅手段
...
PUCT 嘅靈感嚟自**多臂賭博機（Multi-Armed Bandit）** 問題。

想像你面前有 $K$ 部老虎機，每部嘅獲勝機率唔同但未知。你嘅目標係最大化總獲勝次數。策略係：
- **利用**：拉睇落最好嗰部
- **探索**：試其他部，可能發現更好嘅
...
由 $\sqrt{\frac{\ln N}{n}}$ 改為 $\frac{\sqrt{N}}{1+n}$
...
$P(s,a)$ 嚟自神經網絡，會隨住訓練改進。呢個令 MCTS 同神經網絡形成正向循環。
...
1. **$P(s,a)$**：「專家話呢步棋有幾好」
2. **$\sqrt{N(s)}$**：「我哋對呢個局面了解幾多」
3. **$1/(1 + N(s,a))$**：「我哋對呢步棋了解幾多」
...
嘗試調整下面嘅 $c_{\text{puct}}$ 參數，觀察佢點樣影響 MCTS 嘅揀選：
...
$c_{\text{puct}}$ 係最重要嘅超參數。實踐入面嘅指導原則：

**1. 同網絡質素相關**
- 網絡好強（高準確率）：可以用較細嘅 $c_{\text{puct}}$
- 網絡較弱：需要較大嘅 $c_{\text{puct}}$ 嚟修正錯誤

**2. 同搜索預算相關**
- 模擬次數多：可以用較大嘅 $c_{\text{puct}}$（有足夠時間探索）
- 模擬次數少：用較細嘅 $c_{\text{puct}}$（快速聚焦）
...
其中 $\eta \sim \text{Dir}(\alpha)$。呢個增加咗探索嘅多樣性。
...
**2. 價值預測嘅不確定性**
考慮 Value Network 嘅預測信心。

**3. 政策目標學習**
直接學習 MCTS 訪問分佈，而唔係淨係策略頭輸出。
...
**時間複雜度**（每次模擬）：
- Selection：$O(\log N)$（樹嘅深度）
- Expansion：$O(A)$（合法動作數，需要神經網絡推理）
- Evaluation：$O(1)$（Value Network）或 $O(T)$（Rollout，$T$ 係遊戲長度）
- Backpropagation：$O(\log N)$

**空間複雜度**：
- 每個節點：$O(A)$（儲存先驗同訪問統計）
- 成棵樹：$O(N \cdot A)$（$N$ 係節點數）

### 與 Minimax 嘅關係

當 $c_{\text{puct}} \to 0$ 且模擬次數 $\to \infty$ 嗰陣，MCTS + PUCT 會近似於 Minimax 搜索。
...
一啲技術：
1. **Policy Network 過濾**：只考慮 $P(s,a) > \epsilon$ 嘅動作
...
1. **公式結構**：$Q + U$，利用項加探索項
2. **各項意義**：$Q$ 係經驗價值，$P$ 係先驗機率，$N$ 控制探索衰減
3. **與 UCB1 嘅關係**：PUCT 加入咗先驗並使用唔同嘅探索項形式
4. **數學推導**：由多臂賭博機到 MCTS 揀選
5. **實際調參**：$c_{\text{puct}}$ 嘅選擇同影響
...
- **下一篇**：[AlphaGo Zero 概述](../alphago-zero) — 由零開始嘅突破
- **上一篇**：[MCTS 與神經網絡嘅結合](../mcts-neural-combo) — 整體架構
- **相關**：[傳統方法嘅極限](../traditional-limits) — 點解需要新方法
