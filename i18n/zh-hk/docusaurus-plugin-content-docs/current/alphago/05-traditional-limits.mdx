...
sidebar_position: 6
title: 傳統方法的極限
description: 從 Minimax 到 MCTS，深入理解為何傳統 AI 方法在圍棋上失敗
...
本文將深入探討這些方法的原理、優缺點，以及它們為何在圍棋上遇到了瓶頸。
...
要達到理想的剪枝效率，需要先搜索「最好的」分支。但要知道哪個分支最好，本身就需要搜索...這是一個雞先定蛋先的問題。
...
**結論：Alpha-Beta 大幅提升了國際象棋 AI，但對圍棋的幫助有限。**
...
即使在 MCTS 中不直接使用評估函數，**模擬的質素**仍然是關鍵瓶頸。
...
1. **Policy Network**：學習「哪裏可能是好棋」，減少有效分支因子
2. **Value Network**：學習「誰更可能贏」，替代手工評估函數
3. **MCTS 整合**：用神經網絡引導搜索，用搜索改進決策

這不是簡單的「用神經網絡替代評估函數」，而是一種全新的架構。
...
- **上一篇**：[圍棋為何難？](../why-go-is-hard) — 狀態空間與評估困難
- **下一篇**：[棋盤狀態表示](../board-representation) — Zobrist Hashing、特徵編碼
- **技術深入**：[MCTS 與神經網絡的結合](../mcts-neural-combo) — AlphaGo 的核心架構
