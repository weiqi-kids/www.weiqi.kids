...
function PolicyHeatmapInner({
  ...
  useEffect(() => {
    ...
    heatmapGroup.selectAll('rect')
      ...
      .on('mouseover', function(event, d) {
        ...
        d3.select(this).style('stroke', 'var(--chart-ink)').attr('stroke-width', 2);
    ...
    stones.forEach(({ x, y, color }) => {
      stoneGroup.append('circle')
        ...
        .style('fill', color === 'black' ? 'var(--chart-stone-black)' : 'var(--chart-stone-white)')
        .style('stroke', color === 'black' ? 'var(--chart-black)' : 'var(--chart-ink)')
        .attr('stroke-width', 1);
    });
    ...
    topN.forEach((move, idx) => {
      ...
      labelGroup.append('circle')
        ...
        .attr('r', cellSize * 0.3)
        .style('fill', 'var(--chart-white-a80)')
        .style('stroke', 'var(--chart-red)')
        .attr('stroke-width', 2);
  ...
  return (
    <div>
      ...
      <div className="d3-legend">
        <div className="d3-legend-item">
          <div className="d3-legend-color" style={{ background: 'var(--chart-heat-1)' }} />
          低機率
        </div>
        <div className="d3-legend-item">
          <div className="d3-legend-color" style={{ background: 'var(--chart-heat-2)' }} />
          中機率
        </div>
        <div className="d3-legend-item">
          <div className="d3-legend-color" style={{ background: 'var(--chart-heat-3)' }} />
          高機率
        </div>
      </div>
    </div>
