...
path: "/knowledge/ai-go/alphago/mcts-neural-combo/"
kind: "knowledge"
order: 15
datePublished: "2024-01-15"
dateModified: "2024-02-22"
keywords: ["MCTS", "蒙地卡羅樹搜索", "PUCT", "AlphaGo", "神經網路", "UCB", "深度學習", "圍棋AI"]
legacySource: "docs/alphago/14-mcts-neural-combo.mdx"
sourceVerbatim: true
hero: "ag-mcts-neural-combo"
heroAlt: "狸貓吉祥物把發光的大腦裝上棋盤節點組成的樹幹，整棵樹的分支隨之亮起"
heroCaption: "神經網路裝進搜尋樹的主幹，讓每個分支的局面都能被評估（示意）"
---

import MCTSTree from '~/components/charts/MCTSTree.jsx';
...
```mermaid
flowchart LR
    subgraph MCTS["MCTS 循環（重複 N 次）"]
        Selection["Selection<br/>選擇"] --> Expansion["Expansion<br/>展開"]
        Expansion --> Simulation["Simulation<br/>模擬"]
        Simulation --> Backprop["Backprop<br/>反向傳播"]
        Backprop --> Selection
    end
```
...
```mermaid
flowchart TB
    subgraph AlphaGo["AlphaGo 搜索架構"]
        subgraph MCTS["MCTS"]
            Selection["Selection<br/>選擇"]
            Expansion["Expansion<br/>展開"]
            Evaluation["Evaluation<br/>評估"]
            Backup["Backup<br/>更新統計"]

            PUCT["PUCT 公式"]
            PolicyNet["Policy Net"]
            ValueNet["Value Net<br/>+ Rollout"]
            UpdateStats["更新統計"]
        ...
        subgraph NN["Neural Networks"]
            PolicyNetwork["Policy Network<br/>π(a|s)<br/>輸出：動作機率"]
            ValueNetwork["Value Network<br/>V(s)<br/>輸出：勝率估計"]
        end
...
<MCTSTree client:visible width={700} height={450} showPUCT={true} interactive={true} />
...
flowchart TB
    ...
    Fan -->|"+ 更大網路、更多訓練"| Lee
    Lee -->|"+ 更多自我對弈"| Master
    Master -->|"+ 移除人類資料、統一網路、移除 Rollout"| Zero
    Zero -->|"+ 泛化到其他遊戲"| AZ
```
...
- **下一篇**：[PUCT 公式詳解](/knowledge/ai-go/alphago/puct-formula/) — MCTS 選擇的數學原理
- **上一篇**：[自我對弈](/knowledge/ai-go/alphago/self-play/) — 自我對弈的機制與效果
- **相關**：[Policy Network 詳解](/knowledge/ai-go/alphago/policy-network/) — 策略網路的架構
...
<FAQ items={[
  { question: "為什麼需要 MCTS，不能只用神經網路嗎？", answer: "神經網路提供快速直覺但可能有盲點，MCTS 提供深入推理來修正錯誤。實驗顯示純神經網路約 3000 Elo，加入 MCTS 後提升到 4500 Elo，搜索對戰術計算和罕見局面特別有價值。" },
  { question: "PUCT 公式如何平衡探索與利用？", answer: "PUCT 結合平均價值 Q(s,a) 和探索加成 U(s,a)。Q 值偏好已知好的走法（利用），U 值偏好訪問次數少的走法（探索）。Policy Network 的先驗機率進一步引導探索方向。" },
  { question: "AlphaGo Zero 為什麼能移除隨機模擬？", answer: "因為 Value Network 的精度已足夠高，直接評估比隨機模擬更準確且更快。這簡化了系統架構，也說明神經網路品質的提升讓搜索次數從 100,000 降到 1,600 仍能更強。" }
]} />
