...
path: "/knowledge/ai-go/alphago/dual-head-resnet/"
kind: "knowledge"
order: 18
datePublished: "2024-01-15"
dateModified: "2024-02-22"
keywords: ["雙頭網路", "殘差網路", "ResNet", "Policy Head", "Value Head", "深度學習", "多任務學習", "卷積神經網路"]
legacySource: "docs/alphago/17-dual-head-resnet.mdx"
sourceVerbatim: true
hero: "ag-dual-head-resnet"
heroAlt: "一條粗管道末端分成上下兩個出口，分別產出棋子與黑白長條，狐狸站在分叉處"
heroCaption: "共享主幹算完後分成兩個頭，一邊輸出下一手，一邊輸出勝率（示意）"
---
...
flowchart TB
    ...
    subgraph Backbone["共享主幹（ResNet）"]
        ResNet["40 個殘差塊，256 通道"]
    end

    Backbone --> PolicyHead
    Backbone --> ValueHead

    subgraph PolicyHead["Policy Head（策略頭）"]
        Policy["策略預測"]
    end

    subgraph ValueHead["Value Head（價值頭）"]
        Value["價值預測"]
    end
...
```mermaid
flowchart TB
    Input["共享主幹輸出（256 × 19 × 19）"]
    Input --> Conv["1×1 卷積（2 通道）"]
    Conv --> BN["Batch Normalization"]
    BN --> ReLU["ReLU"]
    ReLU --> Flatten["展平（2 × 19 × 19 = 722）"]
    Flatten --> FC["全連接層（362）"]
    FC --> Softmax["Softmax"]
    Softmax --> Output["輸出：362 個機率<br/>（361 個位置 + Pass）"]
```
...
flowchart TB
    Input["共享主幹輸出（256 × 19 × 19）"]
    Input --> Conv["1×1 卷積（1 通道）"]
    Conv --> BN["Batch Normalization"]
    BN --> ReLU1["ReLU"]
    ReLU1 --> Flatten["展平（1 × 19 × 19 = 361）"]
    Flatten --> FC1["全連接層（256）"]
    FC1 --> ReLU2["ReLU"]
    ReLU2 --> FC2["全連接層（1）"]
    FC2 --> Tanh["Tanh"]
    Tanh --> Output["輸出：勝率 [-1, 1]"]
```
...
flowchart TB
    ...
    InitConv --> ResBlocks
    ResBlocks --> Heads["Policy Head / Value Head"]
```
...
- **上一篇**：[AlphaGo Zero 概述](/knowledge/ai-go/alphago/alphago-zero/) — 為什麼不需要人類棋譜
- **下一篇**：[從零訓練的過程](/knowledge/ai-go/alphago/training-from-scratch/) — Day 0-3 的詳細演進
- **技術深入**：[CNN 與圍棋的結合](/knowledge/ai-go/alphago/cnn-and-go/) — 為什麼 CNN 適合棋盤
