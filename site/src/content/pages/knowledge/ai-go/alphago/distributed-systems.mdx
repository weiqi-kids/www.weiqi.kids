...
path: "/knowledge/ai-go/alphago/distributed-systems/"
kind: "knowledge"
order: 20
datePublished: "2024-01-15"
dateModified: "2024-02-22"
keywords: ["分散式系統", "TPU", "並行計算", "MCTS", "虛擬損失", "深度學習", "硬體加速", "批次推理"]
legacySource: "docs/alphago/19-distributed-systems.mdx"
sourceVerbatim: true
hero: "ag-distributed-systems"
heroAlt: "狐狸吉祥物站在推車上為整面伺服器機櫃插入發光運算卡，指示燈連成光帶"
heroCaption: "訓練與推理的運算量，由成排機器和專用加速卡一起分攤"
---
...
```mermaid
flowchart TB
    subgraph Stage1["階段 1：監督學習"]
        Data1["人類棋譜<br/>(3000萬局)"] --> GPU1["GPU 集群<br/>(50 GPUs)"] --> Policy1["Policy Net<br/>(SL版)"]
    end

    subgraph Stage2["階段 2：強化學習"]
        Data2["自我對弈<br/>(數百萬局)"] --> GPU2["GPU 集群<br/>(50 GPUs)"] --> Policy2["Policy Net<br/>(RL版)"]
    end

    subgraph Stage3["階段 3：價值網路訓練"]
        Data3["RL對弈資料<br/>(3000萬局面)"] --> GPU3["GPU 集群<br/>(50 GPUs)"] --> Value["Value Net"]
    end
...
```mermaid
flowchart TB
    subgraph Loop["AlphaGo Zero 訓練循環"]
        SelfPlay["Self-play Workers<br/>(TPU × N)"]
        Buffer["Replay Buffer<br/>(RAM/SSD)<br/>最近 50 萬局"]
        Training["Training Workers<br/>(TPU × M)"]
        Checkpoint["Network Checkpoint"]

        SelfPlay -->|"產生對弈資料"| Buffer
        Buffer -->|"取樣訓練資料"| Training
        Training -->|"更新權重"| Checkpoint
        Checkpoint -->|"最新網路"| SelfPlay
    end
```
...
flowchart TB
    subgraph TPU["TPU v2/v3"]
        subgraph MXU["MXU (128×128)"]
            Matrix["Matrix Multiply Unit<br/>(128×128 = 16K MACs/cycle)"]
        end
        Vector["Vector Unit"]
        HBM["HBM<br/>(16-32 GB)"]
    end
...
flowchart TB
    Root["Root"]
    Root --> Sim1["Sim 1<br/>(獨立)"]
    Root --> Sim2["Sim 2<br/>(獨立)"]
    Root --> Sim3["Sim 3<br/>(獨立)"]
    Root --> Sim4["Sim 4<br/>(獨立)"]
...
```mermaid
flowchart TB
    subgraph Inference["分散式推理架構"]
        Master["主控節點<br/>接收對手落子，發送 AlphaGo 落子"]
        Master --> Controller["MCTS 控制器<br/>管理搜索樹、分配任務、收集結果"]

        subgraph TPUCluster["TPU 集群（48 個 TPU）"]
            TPU1["TPU 1"]
            TPU2["TPU 2"]
            TPU3["TPU 3"]
            TPU4["TPU 4"]
        ...
        TPUCluster -->|"批次處理神經網路推理請求"| Controller
    end
```
...
- **上一篇**：[從零訓練的過程](/knowledge/ai-go/alphago/training-from-scratch/) — 訓練曲線的詳細分析
- **下一篇**：[AlphaGo 的遺產](/knowledge/ai-go/alphago/legacy-and-impact/) — AlphaGo 對 AI 領域的深遠影響
- **相關文章**：[MCTS 與神經網路的結合](/knowledge/ai-go/alphago/mcts-neural-combo/) — MCTS 的基礎知識
...
<FAQ items={[
  { question: "什麼是虛擬損失（Virtual Loss），它如何讓 MCTS 並行化？", answer: "虛擬損失是一種技術，當一個執行緒正在探索某個節點時，臨時降低該節點的價值，讓其他執行緒選擇不同路徑。這樣可以同時收集多個待評估的葉節點，進行批次神經網路推理，大幅提升效率。探索完成後虛擬損失會被真實值覆蓋，不影響最終結果。" },
  { question: "為什麼 AlphaGo 需要 TPU 而不是 GPU？", answer: "圍棋 AI 的計算瓶頸是神經網路推理（佔 95%），每步 MCTS 需要執行 1600 次推理。TPU 是 Google 專為矩陣運算設計的加速器，其 MXU 單元特別擅長這類計算，功耗更低、延遲更少。從 AlphaGo Lee 的 48 TPU 到 AlphaGo Zero 的 4 TPU，效率提升超過 10 倍。" },
  { question: "AlphaGo Zero 的完整訓練成本大約是多少？", answer: "以 Google Cloud 定價估算，每天約 $3,700（4 TPU v2 Pod + 高記憶體 VM + 儲存空間），完整 40 天訓練約 $150,000。但這只是雲端租用成本，DeepMind 作為 Google 子公司可能有內部折扣。對比之下，培養一位職業棋手需要 10-15 年和數百萬的總成本。" }
]} />
