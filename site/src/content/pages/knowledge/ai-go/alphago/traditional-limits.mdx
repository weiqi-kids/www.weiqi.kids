...
path: "/knowledge/ai-go/alphago/traditional-limits/"
kind: "knowledge"
order: 6
datePublished: "2024-01-15"
dateModified: "2024-02-22"
keywords: ["Minimax", "Alpha-Beta剪枝", "MCTS", "蒙地卡羅樹搜索", "電腦圍棋歷史", "UCT演算法", "博弈樹"]
legacySource: "docs/alphago/05-traditional-limits.mdx"
sourceVerbatim: true
hero: "ag-traditional-limits"
heroAlt: "狐狸吉祥物推著冒煙的老舊齒輪計算機卡在陡坡上，狸貓在坡頂搖頭"
---
...
如[上一篇](/knowledge/ai-go/alphago/why-go-is-hard/)所述，圍棋的分支因子約為 250。要看 N 步棋：
...
```mermaid
flowchart TB
    A["Max 節點 A<br/>當前最佳 α = +5"]
    A --> Branch1["分支 1<br/>已評估，值 = +5"]
    A --> B["分支 2：Min 節點 B"]
    B --> B1["子分支 B1<br/>值 = +3"]
    B --> B2["子分支 B2<br/>？？？（可以跳過！）"]

    style B2 fill:#f99,stroke:#f00,stroke-dasharray: 5 5
```
...
<MCTSTree client:visible showPUCT={true} width={700} height={450} />
...
- **上一篇**：[圍棋為什麼難？](/knowledge/ai-go/alphago/why-go-is-hard/) — 狀態空間與評估困難
- **下一篇**：[棋盤狀態表示](/knowledge/ai-go/alphago/board-representation/) — Zobrist Hashing、特徵編碼
- **技術深入**：[MCTS 與神經網路的結合](/knowledge/ai-go/alphago/mcts-neural-combo/) — AlphaGo 的核心架構
...
<FAQ items={[
  { question: "Alpha-Beta 剪枝為什麼不能解決圍棋的搜索問題？", answer: "Alpha-Beta 可以將有效分支因子從 b 減少到約 sqrt(b)。但圍棋的 sqrt(250) 仍約為 16，看 10 步仍需評估 10^12 個節點。而且要達到理想剪枝效率，需要先搜索「最好的」分支，這本身就需要好的評估。" },
  { question: "MCTS 相比 Minimax 有什麼優勢？", answer: "MCTS 不需要評估函數，透過隨機模擬到終局來估計勝率。它能漸進式聚焦有希望的分支，可以隨時停止並給出當前最佳答案。這讓圍棋程式從業餘 10 級提升到業餘高段。" },
  { question: "為什麼需要神經網路來解決評估問題？", answer: "手工特徵無法完整描述人類的圍棋直覺，特徵之間的複雜交互難以建模。深度神經網路可以自動從資料中學習特徵，進行端到端的非線性映射，這正是處理圍棋複雜局面評估所需要的能力。" }
]} />
