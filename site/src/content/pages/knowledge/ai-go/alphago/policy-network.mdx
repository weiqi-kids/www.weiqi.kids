...
path: "/knowledge/ai-go/alphago/policy-network/"
kind: "knowledge"
order: 8
datePublished: "2024-01-15"
dateModified: "2024-02-22"
keywords: ["Policy Network", "策略網路", "卷積神經網路", "CNN", "AlphaGo", "Softmax", "監督學習", "強化學習"]
legacySource: "docs/alphago/07-policy-network.mdx"
sourceVerbatim: true
hero: "ag-policy-network"
heroAlt: "狸貓吉祥物指著棋盤上三個發光的候選落點，狐狸吉祥物在旁睜大眼睛看著"
---

import PolicyHeatmap from '~/components/charts/PolicyHeatmap.jsx';
...
<PolicyHeatmap client:visible initialPosition="corner" size={400} />
...
輸入是 **19×19×48** 的特徵張量：
- **19×19**：棋盤大小
- **48**：48 個特徵平面（詳見 [輸入特徵設計](/knowledge/ai-go/alphago/input-features/)）
...
<PolicyHeatmap client:visible initialPosition="opening" size={400} />
...
<PolicyHeatmap client:visible initialPosition="fighting" size={400} />
...
<PolicyHeatmap client:visible initialPosition="endgame" size={400} />
...
- **下一篇**：[Value Network 詳解](/knowledge/ai-go/alphago/value-network/) — AlphaGo 如何評估局面
- **相關主題**：[輸入特徵設計](/knowledge/ai-go/alphago/input-features/) — 48 個特徵平面詳解
- **深入原理**：[CNN 與圍棋的結合](/knowledge/ai-go/alphago/cnn-and-go/) — 為什麼卷積神經網路適合棋盤
...
<KeyTakeaway>
本文重點：
- Policy Network 是 13 層 CNN，輸入 19x19x48 的特徵張量，輸出 362 個位置的機率分布（含 pass）
- 訓練達到 57% 的 top-1 準確率（預測人類下一步），將 MCTS 搜索空間縮小約一萬億倍
- 強化學習微調讓 Policy Network 從「模仿人類」進化到「追求勝利」，能下出人類未曾想過的創新著法
</KeyTakeaway>

<FAQ items={[
  { question: "為什麼 Policy Network 使用 CNN 而不是其他架構？", answer: "圍棋棋盤是二維網格，局部模式（如眼位、斷點）非常重要。CNN 的卷積核可以有效捕捉這些局部模式，且透過多層堆疊可以獲得大的感受野來理解全局。這與圍棋「局部到全局」的思考方式非常吻合。" },
  { question: "為什麼只有 57% 準確率但棋力很強？", answer: "57% 準確率已經遠超之前的程式（44%），且很多局面有多步好棋，模型選了另一步好棋也會被算「錯」。更重要的是，Policy Network 與 MCTS 結合後，可以透過深度搜索發現更好的選擇。" },
  { question: "輕量版 Policy Network 有什麼用？", answer: "輕量版使用線性模型，速度比完整版快 1500 倍（2 微秒 vs 3 毫秒）。它用於 MCTS 的隨機模擬（rollout）階段，需要從當前節點下到終局，可能需要 100+ 步，速度比精度更重要。" }
]} />
