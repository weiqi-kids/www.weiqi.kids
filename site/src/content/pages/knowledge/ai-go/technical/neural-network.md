...
path: "/knowledge/ai-go/technical/neural-network/"
kind: "knowledge"
order: 4
...
<p class="key-answer" data-question="KataGo 的神經網路架構是什麼？">KataGo 採用單一神經網路、多頭輸出的設計：以殘差塔（20–60 個殘差塊並加入全局池化層）為主幹，從輸入特徵平面提取特徵，再分出 Policy（落子機率）、Value（勝率）、Score（目數差）與 Ownership（領地歸屬）四個輸出頭。</p>
...
flowchart TB
    ...
    Residual["殘差塔<br/>20-60 個殘差塊<br/>+ 全局池化層"]

    Input --> Conv --> Residual

    Residual --> PolicyHead["Policy Head"]
    Residual --> ValueHead["Value Head"]
    Residual --> ScoreHead["Score Head"]
    Residual --> OwnershipHead["Ownership Head"]
...
- [MCTS 實作細節](/knowledge/ai-go/technical/mcts-implementation/) — 搜索與神經網路的結合
- [KataGo 訓練機制解析](/knowledge/ai-go/technical/training/) — 網路如何訓練
- [關鍵論文導讀](/knowledge/ai-go/technical/papers/) — 原始論文的數學推導
