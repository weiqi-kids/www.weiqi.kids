---
title: "關鍵論文導讀"
description: "AlphaGo、AlphaZero、KataGo 等圍棋 AI 里程碑論文的重點解析"
path: "/knowledge/ai-go/technical/papers/"
kind: "knowledge"
order: 11
...
<p class="key-answer" data-question="圍棋 AI 有哪些必讀的關鍵論文？">圍棋 AI 發展史上的里程碑論文包括：Coulom 的 MCTS（2006）、AlphaGo（2016）、AlphaGo Zero（2017）、通用化的 AlphaZero（2017），以及 David Wu 提出多項效率改進的 KataGo（2019）；想了解基礎可先讀 AlphaGo，實作參考則以 KataGo 論文為主。</p>
...
```mermaid
flowchart LR
    A[2006<br/>Coulom<br/>MCTS 首次應用於圍棋] --> B[2016<br/>Silver et al.<br/>AlphaGo Nature]
    B --> C[2017<br/>Silver et al.<br/>AlphaGo Zero Nature]
    C --> D[2017<br/>Silver et al.<br/>AlphaZero]
    D --> E[2019<br/>Wu<br/>KataGo]
    E --> F[2020+<br/>各種改進與應用]
```
...
flowchart TB
    subgraph AlphaGo[AlphaGo 架構]
        ...
        subgraph MCTS[MCTS]
            MCTS1[用 Policy Network 引導搜索]
            MCTS2[用 Value Network + Rollout 評估]
        end
    end
```
...
```mermaid
flowchart TB
    Input[輸入 17 平面] --> ResNet[殘差塔<br/>19 or 39 層]
    ResNet --> Policy[Policy<br/>361]
    ResNet --> Value[Value<br/>1]
```
...
```mermaid
flowchart TB
    Init[初始化隨機網路] --> SelfPlay
    SelfPlay[自我對弈產生棋譜] --> Train
    Train[訓練神經網路<br/>Policy: 最小化交叉熵<br/>Value: 最小化 MSE] --> Eval
    Eval[評估新網路<br/>若較強則替換] --> SelfPlay
```
...
```mermaid
flowchart LR
    A[3 小時<br/>初學者] --> B[24 小時<br/>超越 AlphaGo Lee]
    B --> C[72 小時<br/>超越 AlphaGo Master]
```
...
- [神經網路架構詳解](/knowledge/ai-go/technical/neural-network/) — 深入理解網路設計
- [MCTS 實作細節](/knowledge/ai-go/technical/mcts-implementation/) — 搜索演算法實作
- [KataGo 訓練機制解析](/knowledge/ai-go/technical/training/) — 訓練流程詳解
