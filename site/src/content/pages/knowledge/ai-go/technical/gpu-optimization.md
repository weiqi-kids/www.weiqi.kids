...
path: "/knowledge/ai-go/technical/gpu-optimization/"
kind: "knowledge"
order: 6
...
<p class="key-answer" data-question="如何優化 KataGo 的 GPU 效能？">KataGo 提供 CUDA、OpenCL、Metal、Eigen 四種計算後端；NVIDIA GPU 用 CUDA 效能最佳，跨廠牌可用 OpenCL，Apple Silicon 用 Metal，純 CPU 則用 Eigen。優化關鍵是選對後端並調校批次大小（nnMaxBatchSize）與執行緒數以充分利用 GPU。</p>
...
- [MCTS 實作細節](/knowledge/ai-go/technical/mcts-implementation/) — 批次推理的需求來源
- [模型量化與部署](/knowledge/ai-go/technical/quantization-deploy/) — 進一步的效能優化
- [完整安裝指南](/knowledge/ai-go/kata-go/hands-on/setup/) — 各平台安裝步驟
