...
path: "/knowledge/ai-go/technical/source-code/"
kind: "knowledge"
order: 2
...
<p class="key-answer" data-question="KataGo 的原始碼結構是怎樣的？">KataGo 原始碼（github.com/lightvector/KataGo）主要分為兩部分：cpp/ 是 C++ 核心引擎，含主程式、圍棋規則（game）、MCTS 搜索（search）與神經網路推理（neuralnet）；python/ 則是 Python 訓練程式碼，含訓練主程式、網路架構定義（model.py）與資料處理。</p>
...
- [KataGo 訓練機制解析](/knowledge/ai-go/technical/training/) — 完整訓練流程
- [參與開源社群](/knowledge/ai-go/technical/contributing/) — 貢獻指南
- [概念速查表](/knowledge/interactive/animations/) — 109 個概念對照
