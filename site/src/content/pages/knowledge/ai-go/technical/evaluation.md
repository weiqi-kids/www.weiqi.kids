---
title: "評估與基準測試"
description: "圍棋 AI 的 Elo 評分系統、對局測試與效能基準測試方法"
path: "/knowledge/ai-go/technical/evaluation/"
kind: "knowledge"
order: 9
...
heroCaption: "對局的勝負換算成 Elo 分數，搜索速度另外用碼表計時"
---

本文介紹如何評估圍棋 AI 的棋力與效能，包括 Elo 評分系統、對局測試方法與標準基準測試。

<p class="key-answer" data-question="如何評估圍棋 AI 的棋力與效能？">評估圍棋 AI 棋力主要採用 Elo 評分系統，透過大量對局計算相對勝率與評分差距，並可搭配 SPRT 等統計方法判斷新模型是否確實更強；效能則以每秒搜索的節點數（visits/playouts）等基準測試衡量。</p>
...
def sprt(wins, losses, elo0=0, elo1=10, alpha=0.05, beta=0.05):
    ...
    elo0: 虛無假設的 Elo 差（通常為 0）
    elo1: 對立假設的 Elo 差（通常為 5-20）
    alpha: 假陽性率
    beta: 假陰性率
    """
    if wins + losses == 0:
    ...
    # 計算對數似然比
    p0 = expected_score(elo1, 0)  # H1 下的預期勝率
    p1 = expected_score(elo0, 0)  # H0 下的預期勝率
...
| 症狀 | 可能原因 | 解決方案 |
|------|---------|---------|
| CPU 100%，GPU 低 | 搜索執行緒不足 | 增加 numSearchThreads |
| GPU 100%，輸出慢 | 批次太小 | 增加 nnMaxBatchSize |
| 記憶體不足 | 模型太大 | 使用較小模型 |
| 速度不穩定 | 溫度過高 | 改善散熱 |
...
- [KataGo 訓練機制解析](/knowledge/ai-go/technical/training/) — 模型是如何訓練的
- [分散式訓練架構](/knowledge/ai-go/technical/distributed-training/) — 大規模評估
- [GPU 後端與優化](/knowledge/ai-go/technical/gpu-optimization/) — 效能調校
