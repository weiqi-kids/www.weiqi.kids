...
path: "/knowledge/ai-go/how-it-works/katago-innovations/"
kind: "knowledge"
order: 3
...
<p class="key-answer" data-question="KataGo 相對 AlphaGo 有哪些創新？">KataGo 相對 AlphaGo 的關鍵創新在於完全開源（MIT 授權）、以約 30 GPU、19 天的高效訓練超越前作，並額外輸出目數、領地與多規則支援，使訓練效率與功能都大幅提升。</p>
...
```mermaid
flowchart TB
    A["棋盤輸入<br/>（19×19×22）"] --> B["初始卷積"]
    B --> C["殘差塔<br/>（20-60 個殘差塊）<br/>+ 全局池化"]
    C --> P["Policy Head"]
    C --> V["Value Head"]
    C --> S["Score Head"]
    C --> O["Ownership Head"]
...
```mermaid
flowchart TB
    Input["局部卷積特徵<br/>(19×19×C)"]
    Input --> Conv["繼續卷積"]
    Input --> Pool["全局平均池化"]
    Pool --> Size["(1×1×C)"]
    Size --> FC["全連接層"]
    Conv --> Merge["融合<br/>(concat 或 加法)"]
    FC --> Merge
    Merge --> Output["融合後特徵"]
```
...
- [概念速查表](/knowledge/interactive/animations/) — 109 個動畫概念
- [30 分鐘跑起第一個圍棋 AI](/knowledge/ai-go/kata-go/hands-on/) — 動手安裝 KataGo
- [KataGo 原始碼導讀](/knowledge/ai-go/technical/source-code/) — 深入程式碼
- [KataGo 訓練機制解析](/knowledge/ai-go/technical/training/) — 自我對弈訓練原理
