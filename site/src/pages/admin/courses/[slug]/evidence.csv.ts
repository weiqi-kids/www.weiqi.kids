...
import { POST_STATUS, label } from '../../../../lib/labels';
import { fmtDateTime } from '../../../../server/admin-view';
...
export const GET: APIRoute = async ({ params, locals, url }) => {
  ...
  for (const m of materials) rows.push([`M-${m.id.slice(0, 8)}`, 'Design', '', `技能單張：${m.title}`, m.version, m.id, slug, '第 1 堂', fmtDateTime(m.updated_at), m.display_name, '', `${origin}/camp/courses/${slug}/materials/`, '不含個資', '']);
  ...
  for (const p of posts) {
    ...
    rows.push([`P-${p.id.slice(0, 8)}`, 'Do', '', p.parent_id ? '論壇回覆' : `論壇文章：${p.title ?? ''}`, p.version, p.id, slug, '', fmtDateTime(p.created_at), p.display_name, '', link, '未檢查', p.status === 'published' ? '' : `狀態：${label(POST_STATUS, p.status)}`]);
  }
  ...
  for (const f of files) rows.push([`F-${f.id.slice(0, 8)}`, 'Do', '', '課程論壇圖片', 1, f.id, slug, '', fmtDateTime(f.created_at), f.display_name, '', `${origin}/camp/courses/${slug}/forum/files/${f.id}/`, '未檢查', `附屬於 P-${f.post_id.slice(0, 8)}`]);
