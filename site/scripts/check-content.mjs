...
//
// 本站擴充：src/pages/**/*.astro 的頁面文案也一併掃描（新站頁面多以 .astro 撰寫）。
...
function scanFile(file) {
  ...
  // .astro：<style>／<script> 區塊不是文案，保留行數後清空
  const prose = file.endsWith('.astro') ? src.replace(/<(style|script)\b[\s\S]*?<\/\1>/g, (m) => m.replace(/[^\n]/g, '')) : src;
  const masked = proseMask(prose);
...
function targetFiles() {
  if (fileArgs.length) return fileArgs.filter((f) => /\.(mdx?|astro)$/.test(f));
  ...
  const sets = [
    ...
    run("git ls-files --others --exclude-standard ':(glob)src/**/*.md' ':(glob)src/**/*.mdx' ':(glob)src/pages/**/*.astro'"),
  ];
  const files = [...new Set(sets.filter(Boolean).join("\n").split("\n"))]
    .filter((f) => /^src\/.*\.mdx?$/.test(f) || /^src\/pages\/.*\.astro$/.test(f));
  return files;
}
