...
<p className="key-answer" data-question="KataGo 的神經網路架構是什麼？">KataGo 採用單一神經網路、多頭輸出的設計：以殘差塔（20–60 個殘差塊並加入全局池化層）為主幹，從輸入特徵平面提取特徵，再分出 Policy（落子機率）、Value（勝率）、Score（目數差）與 Ownership（領地歸屬）四個輸出頭。</p>
