...
<p className="key-answer" data-question="如何量化並部署 KataGo 模型？">量化透過降低數值精度減少模型大小與加速推理：FP16 可省一半記憶體且幾乎無精度損失，INT8 可再壓縮至四分之一；量化方式分訓練後量化（PTQ，簡單快速）與量化感知訓練（QAT，精度更高但需重新訓練），再搭配 TensorRT 等工具部署到各平台。</p>
