...
<p className="key-answer" data-question="KataGo 的 MCTS 是如何實作的？">KataGo 的 MCTS 由選擇（Selection）、展開（Expansion）、評估（Evaluation）、回傳（Backpropagation）四步驟循環組成，選擇階段以 PUCT 公式平衡探索與利用，並透過虛擬損失、批次神經網路評估與多執行緒平行化大幅提升搜索效率。</p>
