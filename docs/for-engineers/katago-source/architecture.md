...
本文深入解析 KataGo 的原始碼結構，幫助你理解其內部實作。無論你是想貢獻程式碼、修改功能，還是將其技術應用到自己的專案，這裡都是必讀的參考資料。
...
│   └── asyncbot.h/cpp     # 非同步機器人
├── neuralnet/
│   ├── nninputs.h/cpp     # 神經網路輸入特徵
│   ├── nneval.h/cpp       # 神經網路評估
...
```mermaid
graph TD
    subgraph KataGoNN["KataGo 神經網路架構"]
...
public:
  // 棋盤狀態
  Color stones[MAX_BOARD_AREA];    // 每個點的顏色
...
- [KataGo Discord](https://discord.gg/bqkZAz3) - 與開發者交流
- [KataGo Training](https://katagotraining.org/) - 查看訓練進度

理解了 KataGo 的原始碼架構後，你已經具備了深入研究或貢獻開源專案的能力。如果想要進一步了解，建議直接閱讀 KataGo 的 GitHub 原始碼，並參與社群討論。
